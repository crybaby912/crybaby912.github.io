...
export const projects: Project[] = [
  {
    ...
    incidents: [
      { title: '秒杀支付页金额为 0，订单号显示 -', symptom: '秒杀下单后支付页依赖 Vuex 的普通订单状态，金额、商品名和订单号全部为空。', rootCause: '秒杀流程没有把订单写入同一份前端状态，同时页面读取了 `orderNumber`，后端真实字段是 `number`。', fix: '进入支付页后按 `orderId` 重新拉取订单详情，并兼容真实金额、商品和订单号字段；回归脚本 20 / 20 通过。', lesson: '路由参数和后端订单详情才是支付页的事实来源，不能把临时前端状态当作账本。' },
      ...
      { title: '图片全部空白，问题不只在前端', symptom: '小程序菜品、套餐和订单图片全部不显示，替换组件样式仍无效。', rootCause: '数据库存在空字段和失效 OSS 链接，base64 占位图在微信环境不稳定，管理端代理还拼出了错误路径。', fix: '补齐真实分类图片，统一下载接口和 HTTP 占位图，所有页面走同一个图片降级函数。', lesson: '资源故障要沿着数据、协议、代理和渲染四层排查，不能只改模板。' },
    ],
    evidence: [
